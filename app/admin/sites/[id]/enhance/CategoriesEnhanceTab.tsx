...
import type { CategoryEnrichmentProposal } from '@/lib/services/ai/enrichCategoriesComplete';

interface Props {
  siteId: string;
}

export default function CategoriesEnhanceTab({ siteId }: Props) {
  const [loading, setLoading] = useState(false);
  const [applying, setApplying] = useState(false);
  const [mode, setMode] = useState<'fill_only_empty' | 'overwrite'>('fill_only_empty');
  const [proposals, setProposals] = useState<CategoryEnrichmentProposal[]>([]);
  ...
  async function handleGenerate() {
    try {
      ...
      const res = await fetch(`/api/admin/sites/${siteId}/enhance/categories`, {
        method: 'POST',
      ...
      setSelected(new Set(data.proposals.map((p: any) => p.term_id)));
    } catch (err: any) {
  ...
  async function handleApply() {
    ...
    try {
      ...
      const selectedProposals = proposals.filter((p) => selected.has(p.term_id));

      const res = await fetch(`/api/admin/sites/${siteId}/enhance/categories/apply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          aiJobId,
  ...
  function toggleSelected(termId: string) {
    setSelected((prev) => {
  ...
  function toggleAll() {
    if (selected.size === proposals.length) {
      ...
      setSelected(new Set(proposals.map((p) => p.term_id)));
    }
  }
            ...
            {proposals.map((proposal) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <input
                    type="checkbox"
                    checked={selected.has(proposal.term_id)}
                    onChange={() => toggleSelected(proposal.term_id)}
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div>
                      ...
                      {proposal.currentSeoTitle && (
                        <div className="mb-2 p-2 bg-gray-800 rounded text-sm text-gray-400">
                          <span className="text-xs text-gray-600">Actuel:</span> {proposal.currentSeoTitle}
                        </div>
                      )}
                      <div className="p-2 bg-blue-900/20 border border-blue-500/30 rounded text-sm text-blue-200">
                        <span className="text-xs text-blue-400">Proposé:</span> {proposal.proposedSeoTitle}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {proposal.currentSeoDescription && (
                        <div className="mb-2 p-2 bg-gray-800 rounded text-sm text-gray-400">
                          <span className="text-xs text-gray-600">Actuel:</span> {proposal.currentSeoDescription}
                        </div>
                      )}
                      <div className="p-2 bg-blue-900/20 border border-blue-500/30 rounded text-sm text-blue-200">
                        <span className="text-xs text-blue-400">Proposé:</span> {proposal.proposedSeoDescription}
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="text-sm font-medium text-gray-400 mb-2">
                        Description longue (visible sur le site)
                      </div>
                      {proposal.currentDescription && (
                        <div className="mb-2 p-3 bg-gray-800 rounded text-sm text-gray-400">
                          ...
                          <div dangerouslySetInnerHTML={{ __html: proposal.currentDescription }} />
                        </div>
                      )}
                      <div className="p-3 bg-blue-900/20 border border-blue-500/30 rounded text-sm text-blue-200">
                        ...
                        <div dangerouslySetInnerHTML={{ __html: proposal.proposedLongDescriptionHtml }} />
                      </div>
                    </div>
                  </div>
                </div>
